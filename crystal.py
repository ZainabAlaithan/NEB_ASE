...
class CRYSTAL(FileIOCalculator):
    ...
    def __init__(self, restart=None, ignore_bad_restart_file=False,
                 label='cry', atoms=None, crys_pcc=False, imagenumber = None , **kwargs):
        ...
        FileIOCalculator.__init__(self, restart, ignore_bad_restart_file,
                                  ...
                                  **kwargs)
        if imagenumber != None:
            self.directory = os.path.join(self.directory,str(self.imagenumber)) 
    def write_crystal_in(self, filename):
        ...
        if p.basis == 'custom':
            outfile.write('END \n')
            basisfile = open(os.path.join(os.path.dirname(self.directory), 'basis'))
        ...
        if self.kpts is not None:
            ...
            outfile.write(str(max(self.kpts))+' ' + str(p.isp*max(self.kpts)) + ' \n')
#            if ispbc[2]:
...
#                              + ' 1 1 \n')
    ...
    def write_input(self, atoms, properties=None, system_changes=None):
        ...
        print(os.path.join(self.directory, 'fort.34'))
        write(os.path.join(self.directory, 'fort.34'), atoms, parallel=False)
        print('crystal input written')
    ...
    def read_results(self):
        ...
        for iline, line in enumerate(self.lines):
            if line.find(estring2) >= 0:
                print('found energy and displacement')
                index_energy = iline
                pos_en = 5
        ...
        # Force line indexes
        fstring = 'CARTESIAN FORCES IN HARTREE/BOHR'
        gradients = []
        for iline, line in enumerate(self.lines):
        ...
        print('i just read forces and i will store them in the results')
